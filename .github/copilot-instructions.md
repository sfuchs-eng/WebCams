...
**OTA Firmware Updates via Reboot-to-OTA Mode** (2026-03): OTA updates triggered after any successful image upload in any mode (CONFIG/CAPTURE/WAIT). When the server responds with `ota.available=true`, metadata is saved to NVS namespace `"ota"` and the device reboots. On next boot, `setup()` detects the pending flag **before** camera/webserver init and enters `MODE_OTA` — a minimal boot with WiFi only. No RemoteLogger in this mode. This eliminates the `async_tcp` task watchdog crash and dual-TLS memory pressure that caused previous inline OTA failures. NVS keys: pending update info, `confFile` (firmware filename for post-reboot confirmation), and failure counter (max 3 retries per firmware file, auto-resets for different files). After flashing, device reboots into normal mode, validates on first successful capture, and sends confirmation to `/ota-confirm.php`. See [OTAManager](EspCamPicPusher/lib/OTAManager/), [OTA_UPDATE_SPECIFICATION.md](EspCamPicPusher/AI/Specs/OTA_UPDATE_SPECIFICATION.md).
...
- OTA via dedicated reboot-to-OTA-mode (no async_tcp conflict, max heap)
- OTA pending state/confirmation/retry-count persisted in NVS across reboots
